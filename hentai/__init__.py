...
class Oppai:
	...
	def login(self, log=True):
		...
		if log is True:
			...
			sp = bs(self.ses().get(self.u+"/jiwa.perak").text)
			for x in sp.findAll("a", href=True):
				if "pageSuggestionsOnLiking" in x["href"] or "/a/profile.php?fan" in x["href"]:
					self.ses().get(self.u+x["href"])
					break
		os.system("clear")
